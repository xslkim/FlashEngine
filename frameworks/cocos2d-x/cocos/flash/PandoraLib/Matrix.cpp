...
bool	MyPoint::bitwise_equal(const MyPoint& p) const
...
float MyPoint::get_length() const
{
	return sqrt( x * x + y * y );
}

void MyPoint::twips_to_pixels()
{
	x = TWIPS_TO_PIXELS(x);
	y = TWIPS_TO_PIXELS(y);
}

void MyPoint::pixels_to_twips()
{
	x = PIXELS_TO_TWIPS(x);
	y = PIXELS_TO_TWIPS(y);
}

void MyPoint::setCocos2dPoint( cocos2d::Vec3& point)
{
	point.x = x;
	point.y = y;
	point.z = 0.f;
...
void	Matrix::transform(MyPoint* result, const MyPoint& p) const
...
void	Matrix::transform_vector(MyPoint* result, const MyPoint& v) const
...
void	Matrix::transform_by_inverse(MyPoint* result, const MyPoint& p) const
