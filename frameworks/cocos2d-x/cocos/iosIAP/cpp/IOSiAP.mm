...
- (void) completeTransaction: (SKPaymentTransaction *)transaction
{
    /*NSString *transactionReceipt = [[NSString alloc] initWithData:transaction.transactionReceipt encoding:NSUTF8StringEncoding];
     ...
     [[SKPaymentQueue defaultQueue] finishTransaction: transaction];*/
    NSString *transactionReceipt = [[NSString alloc] initWithData:transaction.transactionReceipt encoding:NSUTF8StringEncoding];
     //transactionReceipt =  [transactionReceipt stringByReplacingOccurrencesOfString:@"+" withString:@"%2B"];
    
    //NSString *url = [[NSString alloc] initWithFormat:@"http://192.168.1.39:8090/icefire/iosreceipt?receipt=%@",transactionReceipt]; 
    ...
    NSURL *sandboxStoreURL = [[NSURL alloc] initWithString: @"http://192.168.1.39:8090/icefire/iosreceipt"];
    NSData *postData = [NSData dataWithBytes:[transactionReceipt UTF8String] length:[transactionReceipt length]];
    ...
    NSData *returnData = [NSURLConnection sendSynchronousRequest:connectionRequest returningResponse:nil error:nil];
...
- (void)paymentQueue:(SKPaymentQueue *)queue removedTransactions:(NSArray *)transactions
{
    //for (SKPaymentTransaction *transaction in transactions) {
    //    std::string identifier([transaction.payment.productIdentifier UTF8String]);
    //    _iosiap->delegate->onPaymentEvent(identifier, IOSIAP_PAYMENT_REMOVED, 1);
    //}
}
