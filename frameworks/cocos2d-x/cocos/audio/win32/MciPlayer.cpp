...
void MciPlayer::SetVolume(float fVolume)
{
    if (! _dev)
    {
        return;
	...
	MCI_DGV_SETAUDIO_PARMS mciSetAudioParms = {0};
	mciSetAudioParms.dwCallback = reinterpret_cast<DWORD_PTR>(_wnd);
	mciSetAudioParms.dwItem = MCI_DGV_SETAUDIO_VOLUME;
	mciSetAudioParms.dwValue = (DWORD)(fVolume * 1000);
	mciSendCommand(_dev, MCI_SETAUDIO, MCI_DGV_SETAUDIO_ITEM | MCI_DGV_SETAUDIO_VALUE, reinterpret_cast<DWORD_PTR>(&mciSetAudioParms));
}
