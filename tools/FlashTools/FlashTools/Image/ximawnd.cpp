...
//bool CxImage::Create(HDC hdc, const TCHAR* text, int len, HFONT hFont, COLORREF cText, COLORREF cBack, bool bSetAlpha/* =false */, int nDisplayWidth /* = 0 */)
//{
//	if(!IsValid())
//	{
//		//get the background
...
//		HFONT pOldFont=NULL;
//		if (hFont)
//			pOldFont = (HFONT)SelectObject(TmpDC, hFont);
//		else
//			pOldFont = (HFONT)SelectObject(TmpDC,GetStockObject(DEFAULT_GUI_FONT));
//		
//		//Set text color
...
//			{
//				int n = 2; 
//				if((char)(text[i]) > 0) //double characater
//				{
//					n = 1;
//				}
//				
//				RECT tRect = {0, 0, 0, 0};
//				::DrawText(TmpDC,text + 2,2,&tRect,DT_CALCRECT);
//				nWidth += tRect.right;
//				if( nWidth >= nDisplayWidth)
//				{
//					pChar[j++] = '\r';
//					pChar[j++] = '\n';
//					nWidth = tRect.right;
//				}
//				memcpy(pChar + j, text + i, n);
//				j += n;
//				i += n;
//			}
//			len = j+1;
//			::DrawText(TmpDC,pChar,len,&pos,DT_CALCRECT);
//
//			drawText = buf;
//		}
//		else
...
//		memset(pbase,0,height*((((24 * width) + 31) / 32) * 4));
//
//		::DrawText(TmpDC,drawText,len,&pos,0);
//
//		this->CreateFromHBITMAP(TmpBmp);
//
//		if(bSetAlpha)
//		{
//			this->AlphaCreate();
...
//			for (long iy=0;iy<height;iy++)
//			{
//				RGBQUAD c = GetPixelColor(ix, iy, false);
//				
//				if(c.rgbRed != 0)
//				{
//					c.rgbReserved = c.rgbRed;
//					c.rgbRed = GetRValue(cText);
//					c.rgbGreen = GetGValue(cText);
//					c.rgbBlue = GetBValue(cText);
//					SetPixelColor(ix, iy, c, bSetAlpha);
//				}
//				else
//				{
//					c.rgbReserved = 0;
//					c.rgbRed = GetRValue(cBack);
//					c.rgbGreen = GetGValue(cBack);
//					c.rgbBlue = GetBValue(cBack);
//					SetPixelColor(ix, iy, c, bSetAlpha);
//				}
//			}
//		}
