...
// added by liudx
bool CxImage::Create(void* pData, long width, long height, COLORREF color, DWORD dwFormat)
{
	if(pData && !IsValid())
	{
		if(dwFormat == 0)
		{
			if(Create(width, height, 24))
			{
				AlphaCreate();
				AlphaSet(0);

				BYTE* pImage = (BYTE*)pData;
				for(long x=0; x<width; x++)
				{
					for(long y=0; y<height; y++)
					{
						...
						SetPixelColor(x, height-1-y, c, true);
					}
				}
				return true;
			}
...
//
//bool CxImage::SplitPowerOf2(LPCTSTR szPrefix, LPCTSTR szSubfix)
//{
...
//	int nIndex = 0;
//	TCHAR szFile[256];
//	strcpy(szFile, szPrefix);
//	TCHAR* pFile = szFile + strlen(szFile);
//	
//
//	long w = lWidth;
//	while(w > 0)
...
//
//			CxImage Img;
//			Img.Create(cx, cy, head.biBitCount);
//
//			if(Crop(x, y, x+cx, y+cy, &Img))
//			{
//				sprintf(pFile, _T("%d%s"), nIndex++, szSubfix);
//				
//				Img.Save(szFile, info.dwType);
//			}
//
//			y += cy;
//		}
