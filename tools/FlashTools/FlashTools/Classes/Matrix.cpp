...
Matrix::Matrix(double aa, double bb, double cc, double dd, double txx, double tyy)
{
	a = aa;
	b = bb;
	c = cc;
...
void	Matrix::concatenate_translation(double tx_, double ty_)
...
void	Matrix::concatenate_scale(double scale)
...
void	Matrix::set_lerp(const Matrix& m1, const Matrix& m2, double t)
...
void	Matrix::set_scale(double x_scale, double y_scale)
{
	a = x_scale;
	b = 0;
	c = 0;
...
void	Matrix::print() const
	// Debug log.
{
	debug_out("| %4.4f %4.4f %4.4f |\n", double(a), double(b), tx);
	debug_out("| %4.4f %4.4f %4.4f |\n", double(c), double(d), ty);
}
...
double	Matrix::get_max_scale() const
	...
	// errors in tesselation.
{
	...
	double	basis0_length2 = a * a + b * b;
	double	basis1_length2 = c * c + d * d;
	double	max_length2 = fmax(basis0_length2, basis1_length2);
	return sqrtf(max_length2);
}

double	Matrix::get_x_scale() const
{
	double scale = sqrtf(a * a + c * c);
	...
	//double scaleX = sqrt( destPoint.x*destPoint.x + destPoint.y*destPoint.y);
	//if( destPoint.x < 0 )
	//{
	//	scaleX = -scaleX;
	//}
...
double	Matrix::get_y_scale() const
{
	return sqrtf(d * d + b * b);
}

void	Matrix::set_scale_rotation(double x_scale, double y_scale, double angle)
...
//
//
//void Matrix::setScaleBias(double xScale, double yScale, double xBias, double yBias)
//{
//	Matrix matX, matY;
//	matX.a = cos(xBias);
//	matX.b = -sin(xBias);
